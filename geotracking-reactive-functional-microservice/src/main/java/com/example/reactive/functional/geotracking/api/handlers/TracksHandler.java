...
public class TracksHandler {
    ...
    public Mono<ServerResponse> getLastPosition(ServerRequest request) {
        ...
        if (deviceId.isPresent() && StringUtils.hasLength(deviceId.get())) {
            return geoPointService.getLastPositionByDeviceId(deviceId.get()).flatMap(geoPoint -> ServerResponse.ok()
                            .body(fromValue(geoPoint)))
                    .switchIfEmpty(ServerResponse.noContent().build());

        } else if (user.isPresent() && StringUtils.hasLength(user.get())) {
            return geoPointService.getLastPositionByUser(user.get()).flatMap(geoPoint -> ServerResponse.ok()
                                    .body(fromValue(geoPoint)))
                            .switchIfEmpty(ServerResponse.noContent().build());
